...
def codegen_type(ty: ir.Attribute, loc: Loc) -> circt.ir.Type:
  ...
  if isinstance(ty, ir.TupleType):
    fields = [
        (f"_{i}", codegen_type(field, loc)) for i, field in enumerate(ty.fields)
    ]
...
class ModuleCodegen:
  ...
  backedges: List[Tuple[CirctValue, ir.SSAValue]] = field(default_factory=list)
  ...
  def codegen_module(self, mod: ir.SiModuleOp):
    ...
    def check_port_name(name: str, loc: Loc):
      if existing := used_names.get(name):
        emit_info(existing, f"previous definition of port `{name}` was here")
