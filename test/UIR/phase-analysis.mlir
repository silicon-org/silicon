...
uir.func @SignaturePhases(%a: -1, %b: 0) -> (result: 0) {
  // CHECK: hir.int_type {phase = "float"}
  %0 = hir.int_type
  // CHECK: hir.int_type {phase = "float"}
  %1 = hir.int_type
  // CHECK: hir.int_type {phase = "float"}
  %2 = hir.int_type
  // CHECK: uir.signature ({{.*}}) -> ({{.*}}) {phase = 0 : si16}
  uir.signature (%0, %1) -> (%2)
} {
  uir.return -> ()
}
...
uir.func @AllBranchesReturn(%cond: 0, %val: 0) -> (result: 0) {
  ...
  // CHECK: uir.unreachable {phase = 0 : si16}
  uir.unreachable
}
...
uir.func @CallConstArg(%n: -1, %a: 0) -> (result: 0) {
  ...
  // CHECK: uir.call @ConstArgTarget({{.*}}) {{.*}} {phase = 0 : si16}
...
uir.func @LoopContinue(%c1: 0, %c2: 0) -> () {
  ...
  uir.loop {
    uir.if %c1 {
      // CHECK: uir.continue {phase = 0 : si16}
      uir.continue
    } else {
      uir.unreachable
    }
...
uir.func @ZeroUseCall(%n: -1, %a: 0) -> () {
  ...
  // CHECK: uir.call @VoidTarget({{.*}}) {{.*}} {phase = 0 : si16}
...
uir.func @ChainedTypeOf(%a: 0) -> (result: 0) {
  ...
  %ta = hir.type_of %a
  // type_of(%ta at -1) = -2.
  // CHECK: hir.type_of {{.*}} {phase = -2 : si16}
  %tta = hir.type_of %ta
...
uir.func @PureTypeOperandEarliest(%a: -1) -> (result: 0) {
  ...
  // CHECK: hir.uint_type {{.*}} {phase = -1 : si16}
  %ut = hir.uint_type %a
...
uir.func @MultiResultCall(%a: -1) -> (r0: 0, r1: 0) {
  ...
  // CHECK: uir.call @MultiResultTarget({{.*}}) {{.*}} {phase = -1 : si16}
...
uir.func @TypeOfMultiConsumer(%a: 0) -> (result: 0) {
  ...
  // CHECK: hir.type_of %a {phase = -1 : si16}
  %ta = hir.type_of %a
  // CHECK: hir.add %a, %a : {{.*}} {phase = 0 : si16}
  %sum1 = hir.add %a, %a : %ta
  // CHECK: hir.add {{.*}}, %a : {{.*}} {phase = 0 : si16}
...
uir.func @CallWithTypeOfResults(%a: 0) -> (result: 0) {
  ...
  %tr = hir.int_type
  // CHECK: uir.call @TypedResultTarget({{.*}}) {{.*}} {phase = 0 : si16}
...
uir.func @ConstantIntTypeOperand() -> (result: 0) {
  ...
  %t = hir.int_type
  // CHECK: hir.constant_int {{.*}} {phase = "float"}
...
uir.func @InferrableFloats() -> (result: 0) {
  ...
  // CHECK: hir.inferrable {phase = "float"}
  %inf = hir.inferrable
  %t = hir.int_type
  uir.return %inf -> (%t)
}
...
uir.func @CoerceTypePhase(%a: 0) -> (result: 0) {
  ...
  %ty = hir.int_type
  // CHECK: hir.coerce_type {{.*}} {phase = 0 : si16}
...
uir.func @CallNonConstTypeOfArgs(%n: -1, %a: 0) -> (result: 0) {
  ...
  %ut = hir.uint_type %n
  %tr = hir.int_type
  // CHECK: uir.call @TypedArgTarget({{.*}}) {{.*}} {phase = 0 : si16}
