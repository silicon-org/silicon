...
- Never run clang-format on `.td` (tablegen) files; it mangles the trait and argument lists, fighting the established vertical layout. Format `.td` files by hand to match the surrounding style.
...
- TODO.md is a current snapshot, not a work log: delete done items, and replace partially-done items with only the work that's left; don't track deltas, changes, or failure-mode explanations -- just enough breadcrumbs to know where to look and what to investigate.
